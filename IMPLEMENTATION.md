...
└── ui/
    ...
    ├── settings_dialog.py       # 设置（主题/主界面与终端字体/语言/编辑器）
    ├── about_dialog.py          # 关于
...
- Session 树和快捷命令树共用同级拖放逻辑，按目标节点上方/下方指示位置插入；根节点向上移动（含首位）不额外递减目标索引，并保留子节点、展开状态和当前选中项。
...
- **上传/下载仅通过右键菜单发起**（不支持文件面板拖拽互传）。默认上传到右侧远端面板当前路径、下载到左侧本地面板当前路径；远端右键菜单另提供“下载到其它位置...”用于选择一次性本地目标目录，不改变左侧面板当前路径。上传/下载开始前，statusbar 先显示正在计算对应批次总大小；本地上传统计通过 `asyncio.to_thread()` 执行，避免目录遍历阻塞界面。统计完成后，上传和下载统一显示速度、批次已传/总大小、截断到一位小数的批次总百分比、方向图标及底部约 4px 进度条，空闲时隐藏。上传与下载独立统计，可双向同时显示，传输文本不显示 tooltip。
...
`ConnectionManager.cd_shell()`：优先等待终端画面出现常见 `user@host:path$`/`#` Prompt（最多 5 秒），再向交互式 Shell 写入一次 `cd <path>\r`，使终端工作目录与文件面板远端路径一致。Prompt 无法识别或启动脚本异常时，额外等待最多 1 秒，并在输出连续静默 0.5 秒后兜底发送。等待结束后重新核对原 Tab 的 SSH Session、Terminal 和文件面板仍然有效，关闭期间的过期初始化不再更新 UI。
...
设置对话框可直接保存主题、语言、`appearance.ui_font_size_px`、终端字体/字号、默认编辑器和远端大文件阈值，并立即刷新当前应用外观。对话框底部提示其余配置需关闭程序后手动编辑 `config/config.json`，避免运行期窗口状态保存覆盖手工修改。

Session 配置远端默认路径时，连接后的交互 shell 会优先等待终端画面出现首个常见 `user@host:path$`/`#` Prompt，再发送一次 `cd`，避免命令在 Prompt 出现前进入 PTY 后被终端 echo 与 readline 重绘成两行。若自定义 Prompt 无法识别或启动脚本异常，则在总超时后等待一次短暂输出静默再兜底发送，避免文件面板初始化永久阻塞。
...
- **远程：** 刷新、新建目录；有选中项时：使用系统关联程序/配置编辑器打开、复制文件名、复制路径（`A`）、复制父路径（`P`）、发送文件名（`S`）/路径（`F`）/父路径（`G`）到对应 Tab 终端、将终端路径切换到当前远端目录（`Q`）、跳转到终端当前路径（`W`）、下载、重命名（单选）、删除。多选复制时各行以换行拼接；发送名称或路径时逐项执行 POSIX shell 引用并以空格拼接，只填入终端而不执行；包含控制字符时拒绝发送。「将终端路径切换到此处」先发送 `Ctrl+A`、`Ctrl+K` 清空整行 shell 输入，再执行经过 POSIX shell 引用的 `cd -- <path>`；包含控制字符的路径不执行。「跳转到终端路径」按 Tab 缓存远端主动发送的标准 `OSC 7` 路径，并在开始输入或菜单触发时尽力解析常见 `user@host:/absolute/path$`/`#` Prompt；命中缓存时不向终端发送内容。仅在两者均未取得路径、Tab 仍连接且终端不在备用屏时，才用 readline `Ctrl+A`/`Ctrl+K` 暂存当前未执行输入，通过同一交互式 POSIX shell 的私有 OSC 回报取得 `$PWD`，再以 `Ctrl+Y` 恢复暂存输入；断连或备用屏无缓存时不主动查询。控制回报不写入终端画面，远端文件面板只切换对应 Tab 的路径，两个 Tab 的缓存和面板路径互相隔离。
- **本地：** 刷新、新建目录；有选中项时：使用系统关联程序/配置编辑器打开、复制文件名、复制路径（`A`）、复制父路径（`P`）、上传、重命名（单选）、移到回收站（Shift 按下时为永久删除）。
- 编辑动作只处理文件并忽略目录；实际文件达到 3 个时先统一确认。

### 远端文件编辑

- `FileEditManager` 将远端文件下载到系统临时目录的本次运行隔离子目录，路径结构为 `remote-edit/<运行实例 ID>/<host>/<Tab 与远端路径摘要>/<安全文件名>`。其中运行实例 ID 是每次启动随机生成的 32 位 UUID 十六进制串（例如 `b1ad016f48b745008654d81a96a4168d`），用于隔离不同应用进程；其后的 20 位十六进制串（例如 `9779de5fe99df880b83b`）是 `tab_id` 与完整远端路径的 SHA-256 摘要前缀，用于隔离不同 Tab 和同名远端文件。`host` 目录让外部编辑器中的本地路径可直接辨认来源服务器；host 和远端文件名都会映射为 Windows 可接受的安全名称，文件名保留常见扩展名。下载后将临时文件 mtime 对齐到远端 mtime；同一 Tab 的同一路径重复打开时会重新读取远端 size/mtime，仅在远端签名未变化时复用临时副本，远端已变化则覆盖下载最新副本后再打开。
...
| 本地文件 IO | 上传读文件、本地删除/重命名/新建目录仍有同步 IO；上传前的本地大小统计已通过 `asyncio.to_thread` 执行，其余待后续迁移或分片让出事件循环 |
